...
export default function AppLayout(props) {
  ...
  function getTablerIcon(tablerIcon) {
    if (!tablerIcon) {
      return <></>;
    }
    const TablerIcon = tablerIcon;
    return <TablerIcon className="w-4 h-4" />;
  }
                ...
                <Menu shadow="md" width={220}>
                  <Menu.Target>
                    <button className={`max-w-full`}>
                      <img src={gridIcon} alt="" className={`max-w-[16px]`} />
                    </button>
                  </Menu.Target>
                  ...
                  <Menu.Dropdown>
                    <div className={`p-2`}>
                      ...
                      <div className={`flex gap-3 flex-wrap justify-center`}>
                        {adjustedApps.map((link, index) => {
                          const userRoleIds =
                            user?.all_roles?.map((rec) => rec.string_id) || [];
                          const isVisible = link.roleIds
                            ? link.roleIds.some((roleId) =>
                          ...
                          return (
                            ...
                            >
                              <Card
                                className={`w-[40px] h-[40px] !rounded-xl hover:-translate-y-0.5 p-0 flex justify-center items-center cursor-pointer ${link.className}`}
                              >
